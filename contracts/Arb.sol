...
contract Arb {
    ...
    // callback
    function receiveLoan(bytes memory data) public returns (bool) {
        require(!lock, "Locked");
        ...
        if (_repay_data.direct_repay) {
            ...
            if (out_amount < _repay_data.repay_amount) {
                console.log("out_amount lt _repay_data.repay_amount No Profit!");
                revert("out_amount lt _repay_data.repay_amount No Profit!");
            }
            IERC20(_repay_data.repay_token).safeTransfer(_repay_data.recipient, _repay_data.repay_amount);
            return true;
        }
